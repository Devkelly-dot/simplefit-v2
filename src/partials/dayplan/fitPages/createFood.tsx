...
type Props = {
    food_to_edit?:foodType;
    onSave: ()=>any;
};

const CreateFood: React.FC<Props> = ({food_to_edit, onSave}) => {
    const dispatch = useDispatch();
    const token = useSelector((state:RootState)=>state.auth.token);
    const selectedDayplan = useSelector((state:RootState)=>state.dayplan.selectedDayplan);

    useEffect(()=>{
    },[token, food_to_edit, selectedDayplan])
        ...
        [
            {
                alias: 'Name',
                key: 'name',
                value: food_to_edit.name,
                component: (value,id, index)=>inputComponent(value, id, onTextChange, index)
            },
            {
                alias: 'Description',
                key: 'description',
                value: food_to_edit.description,
                component: (value, id, index)=>inputComponent(value,id, onTextChange, index)
            },
            {
                alias: 'Complete',
                key: 'complete',
                value: food_to_edit.complete,
    ...
    async function handleSubmit(e)
    {
        ...
        if(food_to_edit)
            submit_form['id'] = food_to_edit.id;
        else submit_form['id'] = -1;

        createFitObject(dispatch, selectedDayplan.id, token, 'food', submit_form)
        onSave();
    }
    return(
        <Card>
            <CardBody>
                {
                    form.map((form_field, index)=>{
                        return<div key={form_field.key}>
                            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor={'food-'+form_field.key}>{form_field.alias}</label>
                            {
                                form_field.component(form_field.value, 'food-'+form_field.key, index)
                            }
                        </div>
                    })
                }
